...
- Fiscal periods come from each database's `SY40100` period rows joined to its `SY40101` fiscal-year bounds. Every period from fiscal 2020 forward whose start is on/before `--as-of` is emitted by default, including the current open/unreconciled month; callers may request a smaller trailing count explicitly. Close status never limits selection. Future, not-yet-started periods are not emitted. Period 0 is not emitted, but its opening debit/credit is included in balance-sheet ending balances. `--as-of` controls calendar availability, not a transaction-date cutoff: reports show actual posted GP period balances at extraction time, not unposted batches or projections.
