...
-- Validate only the requested immutable run. The original definition scanned
-- payload/trend JSON for every historical run, causing statement timeouts as
...
begin
  ...
  if exists (
    ...
    where p.run_id = p_run_id
      and (
        pg_catalog.jsonb_typeof(p.payload#>'{statement,trend}') <> 'array'
